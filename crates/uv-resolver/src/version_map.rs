    ...
    fn get_simple<'p>(
        ...
        let get_or_init = || {
            ...
            for (filename, mut file) in files.all(&self.package_name) {
                ...
                match prepare_registry_file(&mut file, &self.index_route, candidate.is_compatible())
                {
                    Ok(true) => {}
                    Ok(false) => continue,
                    Err(error) => {
...
/// Prepare a proxy artifact after its compatibility with the resolution is known.
///
/// Unmapped, incompatible advertisements cannot be used as wheel metadata for a compatible source;
/// retaining them would attach a physical URL to a canonical distribution. Mapped incompatible
/// artifacts remain available for ordinary resolver diagnostics and marker handling. Advertised
/// hashes are intentionally left to lock construction because live `pip` installs may be hashless.
fn prepare_registry_file(
    file: &mut File,
    index_route: &IndexRoute,
    is_compatible: bool,
) -> Result<bool, ProxyIndexError> {
    if !index_route.is_proxy() {
        return Ok(true);
    }

    match canonicalize_registry_file(file, index_route) {
        Ok(()) => {}
        Err(_) if !is_compatible => return Ok(false),
...
/// Restore canonical artifact identity without modifying cached Simple metadata.
fn canonicalize_registry_file(
    ...
    if index_route.is_proxy() {
        ...
        // Proxy artifact routes download ordinary wheel bytes, not compressed alternatives.
        file.zstd = None;
    }
...
mod tests {
    ...
    use uv_distribution_types::{Index, IndexLocations, IndexName, IndexRoutes, Zstd};
    use uv_pypi_types::{HashDigest, HashDigests};
    use uv_redacted::DisplaySafeUrl;
    use uv_small_str::SmallString;

    use super::{
        File, FileLocation, IndexRoute, IndexUrl, ProxyIndexError, canonicalize_registry_file,
        prepare_registry_file,
    };
    ...
    #[test]
    fn canonicalize_proxy_wheel_discards_unused_zstd_metadata()
    -> Result<(), Box<dyn std::error::Error>> {
        let mut file = registry_file(
            "https://proxy.example.com/files/example-1.0.0-py3-none-any.whl",
            "example-1.0.0-py3-none-any.whl",
        ...
        expected.url = FileLocation::AbsoluteUrl(
            DisplaySafeUrl::parse(
                "https://canonical.example.com/packages/example-1.0.0-py3-none-any.whl",
            )?
            .into(),
        );
        expected.zstd = None;
    ...
    #[test]
    fn direct_compressed_artifact_preserves_metadata_and_hashes()
    -> Result<(), Box<dyn std::error::Error>> {
        let canonical = IndexUrl::from_str("https://canonical.example.com/simple/")?;
        let route = IndexRoutes::default().route_for(&canonical);
        let mut file = registry_file(
            "https://canonical.example.com/packages/example-1.0.0-py3-none-any.whl",
            "example-1.0.0-py3-none-any.whl",
        );
        file.hashes = HashDigests::from(vec![
        ...
        canonicalize_registry_file(&mut file, &route)?;
        assert!(prepare_registry_file(&mut file, &route, true)?);
        assert_eq!(file, original);
        Ok(())
    }

    #[test]
    fn prepare_proxy_artifacts_respects_mapping_and_compatibility()
    -> Result<(), Box<dyn std::error::Error>> {
        let route = proxy_route()?;

        for (origin, compatible, expected) in [
            ("https://proxy.example.com/files/", true, Some(true)),
            ("https://proxy.example.com/files/", false, Some(true)),
            ("https://unmapped.example.com/files/", false, Some(false)),
            ("https://unmapped.example.com/files/", true, None),
        ] {
            let filename = if compatible {
                "example-1.0.0-py3-none-any.whl"
            } else {
                "example-1.0.0-cp39-cp39-win_amd64.whl"
            };
            let mut file = registry_file(&format!("{origin}{filename}"), filename);
            let original = file.clone();

            match (
                expected,
                prepare_registry_file(&mut file, &route, compatible),
            ) {
                (Some(expected), Ok(actual)) => {
                    ...
                    if expected {
                        assert_eq!(
                            file.url,
                            FileLocation::AbsoluteUrl(
                                DisplaySafeUrl::parse(&format!(
                                    "https://canonical.example.com/packages/{filename}"
                                ))?
                                .into()
                            )
                        );
                ...
                (None, Err(ProxyIndexError::UnmappedUrl { .. })) => {
                    assert_eq!(file, original);
                }
                (_, Err(error)) => return Err(error.into()),
                (None, Ok(_)) => return Err("a compatible unmapped artifact was accepted".into()),
            }
        }
